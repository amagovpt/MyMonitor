...
import { SuccessfulUploadManualEvaluationDialog } from "./modal/successfulUploadManualEvaluationDialog";

import { api } from '../../config/api'

export default function UploadManualEvaluation() {
  ...
  const pageTitle = `${t("UPLOAD_MANUAL_EVALUATION.title")} ${name}`
  ...
  const breadcrumbs = [
    ...
    {
      title: t("UPLOAD_MANUAL_EVALUATION.title") ,
    }
  ];
    ...
    <>
      ...
      <div className={`container ${homeDark}`}>
        ...
        <div className="d-flex flex-row justify-content-between mb-3 align-items-center ">
          <h1>{t("UPLOAD_MANUAL_EVALUATION.title")}</h1>
        </div>
        <div className={`${homeDarkSection} p-5`}>
          <p className={`${textColorTheme} ama-typography-body-large mb-4`}>{t("UPLOAD_MANUAL_EVALUATION.tutorial_text")}</p>
          <ChooseFile data={data} openDialog={openDialog} setOpenDialog={setOpenDialog} />
          <Modal
            isOpen={openDialog}
            onRequestClose={() => setOpenDialog(false)}
          >
            <SuccessfulUploadManualEvaluationDialog close={() => setOpenDialog(false)} />
          </Modal>
        </div>
      </div>
    </>
